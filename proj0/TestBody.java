...
public class TestBody {
    ...
    private static void checkEquals(double actual, double expected, String label, double eps) {
        if (approxEqual(actual, expected, eps)) {
            System.out.println("PASS: " + label
                    + ": body 1 to body 2 is" + expected + " and you gave " + actual);
        } else {
    ...
    private static void checkTwo() {
        ...
        Body b2 = new Body(2.0, 3.0, 3.0, 4.0, 4e11, "jupiter.gif");


        checkEquals(b1.calcForceExertedByX(b2), -b2.calcForceExertedByX(b1), "calcForceExertedByX()", 0.01);
        checkEquals(b1.calcForceExertedByY(b2), -b2.calcForceExertedByY(b1), "calcForceExertedByY()", 0.01);
